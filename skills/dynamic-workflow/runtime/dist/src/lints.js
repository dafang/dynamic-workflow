...
function* walkSteps(steps, prefix = "steps") {
...
function warnCurrentAgentStub(step, stepPath, warnings) {
    if (!isAgentStepType(step.type))
        return;
    if (step.input?.agent_backend === "paseo")
        return;
    warnings.push({
...
function warnBroadRg(commands, step, stepPath, warnings) {
...
function warnNestedShell(commands, step, stepPath, warnings) {
    for (const [commandIndex, command] of commands.entries()) {
...
function warnVerifyOptionalSearches(commands, step, stepPath, warnings) {
...
function warnOversizedCommandGroup(commands, step, stepPath, warnings) {
