...
export const AGENT_OUTPUT_CONTRACTS = {
    "agent.classify": {
        type: "agent.classify",
...
export function isAgentStepType(type) {
    return AGENT_STEP_TYPES.includes(type);
}
export function getAgentOutputContract(type) {
    return AGENT_OUTPUT_CONTRACTS[type];
}
export function agentContractSchemaAsJson(type) {
    return AGENT_OUTPUT_CONTRACTS[type].schema;
}
export function buildAgentOutputInstructions(type, extraSchema) {
...
export function validateAgentOutput(type, output, extraSchemas = []) {
    const schemas = [
        getAgentOutputContract(type).schema,
        ...extraSchemas.map((schema) => schema)
    ];
