...
export async function runWorkflow(planInput, options = {}) {
    ...
    while (record.state === "running") {
        ...
        for (const stepId of ready) {
            ...
            const result = await buildStepContext({
                ...
                output: { status: "failed", step_id: stepId, reason: "context_error", message: error.message }
            }));
            ...
            await store.saveRun(record);
            if (options.stopAfterStepId === stepId || record.state !== "running")
                break;
        }
    }
...
async function writeSkippedStepArtifact(artifactStore, stepId, summary, condition, record) {
...
async function readRunIfSourceArtifact(condition, record) {
    const dependency = record.steps[condition.step];
    if (!dependency?.output_path)
        return undefined;
    return readJson(dependency.output_path).catch(() => undefined);
}
async function enforceAgentOutputContract(node, result, trace) {
    if (!isAgentStepType(node.type) || result.status !== "succeeded") {
        return result;
    }
    const schemas = [recordValue(node.input.output_schema), node.verify?.output_schema].filter((schema) => schema !== undefined);
...
export function createRunId(plan) {
    const suffix = Date.now().toString(36);
    return `${plan.workflow_id.replace(/[^A-Za-z0-9_-]/g, "_")}_${suffix}`;
}
async function evaluateRunIf(condition, record) {
    const dependency = record.steps[condition.step];
    if (!dependency?.output_path)
        return false;
