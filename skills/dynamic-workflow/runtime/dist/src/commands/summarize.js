...
export async function summarizeCommand(args, context) {
    ...
    try {
        ...
        const steps = await Promise.all(Object.values(record.steps).map(async (step) => {
            const artifact = step.output_path ? await readJson(step.output_path).catch(() => undefined) : undefined;
            return {
