...
import type { StepConsume, WorkflowPlan, WorkflowStep } from "./types.js";
...
function rejectUnsupportedSyntax(code: string): void {
  const unsupported: Array<{ name: string; pattern: RegExp }> = [
...
function usesDeclarativeDsl(source: string): boolean {
  return /\bcommand\s*\(/.test(source) || /\bagent\.(?:review|synthesize|execute)\s*\(/.test(source);
}

function captureDeclarativeSteps(source: string): WorkflowStep[] {
  const steps: WorkflowStep[] = [];
  const handles = new Map<string, CapturedHandle>();
  const seenStepIds = new Map<string, number>();
  let position = 0;
...
interface DeclarativeAssignment {
  variable?: string;
  kind: "command" | "review" | "synthesize" | "execute";
  callStart: number;
  openParen: number;
}

function findNextDeclarativeAssignment(source: string, start: number): DeclarativeAssignment | undefined {
  const pattern =
    /(?:(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*)?(command|agent\.(?:review|synthesize|execute))\s*\(/gm;
  pattern.lastIndex = start;
  const match = pattern.exec(source);
  if (!match) return undefined;
  const kindText = match[2] ?? "";
  const kind = kindText === "command" ? "command" : (kindText.slice("agent.".length) as "review" | "synthesize" | "execute");
  const assignment: DeclarativeAssignment = {
    kind,
...
function findCallExpressionEnd(source: string, callStart: number): number {
  const openParen = source.indexOf("(", callStart);
  if (openParen === -1) throw new Error("Harness unsupported syntax: call_expression");
  let depth = 0;
  let index = openParen;
  while (index < source.length) {
...
function createDeclarativeStep(
  kind: DeclarativeAssignment["kind"],
  stepId: string,
  options: string,
  handles: Map<string, CapturedHandle>
): WorkflowStep {
  if (kind === "command") {
  ...
  const type =
    kind === "review" ? "agent.review" : kind === "synthesize" ? "agent.synthesize" : "agent.execute";
  const permissionProfile =
    type === "agent.review" ? "reviewer_readonly" : type === "agent.synthesize" ? "synthesizer" : "executor_writer";
  const step: WorkflowStep = {
    step_id: stepId,
    type,
...
function readFirstStringArg(args: string): string | undefined {
  return args.match(/^\s*["'`]([^"'`]+)["'`]/)?.[1];
}

function readSecondObjectArg(args: string): string {
...
function readStringArrayOption(options: string, key: string): string[] {
  const match = options.match(new RegExp(`${key}\\s*:\\s*\\[([\\s\\S]*?)\\]`, "m"));
  if (!match?.[1]) return [];
  return [...match[1].matchAll(/["'`]([^"'`]+)["'`]/g)].map((entry) => entry[1] ?? "");
}

function readProducesOption(options: string): WorkflowStep["produces"] | undefined {
  const body = readObjectOption(options, "produces");
  if (!body) return undefined;
  const produces: WorkflowStep["produces"] = {};
  for (const match of body.matchAll(/([A-Za-z_][A-Za-z0-9_.-]*)\s*:\s*["'`]([^"'`]+)["'`]/g)) {
    const name = match[1];
    const select = match[2];
    if (name && select) produces[name] = { select };
  }
  return Object.keys(produces).length > 0 ? produces : undefined;
}

function readContextConsumes(options: string, handles: Map<string, CapturedHandle>): StepConsume[] {
...
function readObjectOption(options: string, key: string): string | undefined {
