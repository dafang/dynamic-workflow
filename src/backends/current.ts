...
async function executeVerifyCommands(node: CompiledNode): Promise<BackendStepResult> {
...
function getVerifyCommands(node: CompiledNode): string[] {
  if (Array.isArray(node.verify?.commands)) {
    return node.verify.commands;
  }
  if (Array.isArray(node.input.commands)) {
    return node.input.commands.filter((command): command is string => typeof command === "string");
  }
  return [];
}

async function runShell(command: string): Promise<JsonObject> {
  try {
