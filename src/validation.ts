...
function validateLoopInput(
  ...
  if (
    !Number.isInteger(loopInput.max_rounds) ||
...
function validateLoopBody(
  ...
  const bodySteps: Array<{ step_id: string; depends_on: string[]; raw: Record<string, unknown>; path: string }> = [];
...
function validateLoopBodyRunIf(
  value: unknown,
  bodyStepId: string,
  bodyIds: Set<string>,
  topLevelDependsOn: Set<string>,
  bodyPath: string,
  loopStepId: string,
  errors: ValidationError[]
): void {
...
function validateLoopBodyConsumes(
  consumes: unknown[],
  bodyStepId: string,
  bodyIds: Set<string>,
  topLevelDependsOn: Set<string>,
...
function validateLoopBodyCycles(
  bodySteps: Array<{ step_id: string; depends_on: string[] }>,
  loopStepId: string,
  errors: ValidationError[]
): void {
...
function validateCommandStepCommands(
  rawStep: Record<string, unknown>,
  path: string,
  stepId: string,
  stepType: "command.verify" | "command.collect",
  errors: ValidationError[]
): void {
  ...
  readCommandDeclarations(
    commands,
    stepType === "command.verify" ? `${path}.verify.commands` : `${path}.collect.commands`,
    stepId,
    errors
  );
}

function readVerificationSpec(
  value: unknown,
  path: string,
  stepId: string,
  errors: ValidationError[]
...
function readCommandCollectionSpec(
  value: unknown,
  path: string,
  stepId: string,
  errors: ValidationError[]
): CommandCollectionSpec | undefined {
  if (!isRecord(value)) return undefined;
  const commands = readCommandDeclarations(value.commands, `${path}.commands`, stepId, errors);
  return commands ? { commands } : undefined;
}

function readCommandDeclarations(
  value: unknown,
  path: string,
  stepId: string,
  errors: ValidationError[]
...
function readExitCodes(value: unknown, path: string, stepId: string, errors: ValidationError[]): number[] | undefined {
  if (value === undefined) return undefined;
  if (!Array.isArray(value) || value.length === 0) {
...
function readPositiveNumber(value: unknown, path: string, stepId: string, errors: ValidationError[]): number | undefined {
  if (value === undefined) return undefined;
